...
object Schema {
   ...
   */
  def schemaFromFields(fields: List[Field]) =
    Root(typ = Some(SimpleTypeTyp(SimpleTypes.Object)), properties = Some(fields))
   ...
   */
  def schemaFromArray(schema: Root) =
    Root(typ = Some(SimpleTypeTyp(SimpleTypes.Array)), items = Some(ItemsRoot(schema)))
  ...
  case class Root($schema: Option[String] = None,
  ...
  private def toJsonList(sl: Option[List[String]]): Option[List[Json]] =
    sl.map(_.map(s => parser.parse(s).toOption.get))
  ...
  implicit val TypDecoder: Decoder[Typ] =
    Decoder.instance { c =>
      (c.as[List[SimpleType]].map(ListSimpleTypeTyp(_))).orElse(c.as[SimpleType].map(SimpleTypeTyp(_)))
    }
  ...
  implicit val ItemsDecoder: Decoder[Items] =
    Decoder.instance { c =>
      (c.as[Root].map(ItemsRoot(_))).orElse(c.as[SchemaArray].map(ItemsSchemaArray(_)))
    }
