...
class ModelBuilder[U <: Universe](val u: U) {
   ...
   */
  def mkCaseClassDef(path: List[String],
                     name: String,
                     fields: List[Field],
                     requiredFields: Option[List[String]],
   ...
   */
  def mkEnumDef(path: List[String],
                baseName: String,
                enum: List[String],
                splitOnUnderscore: Boolean = false
  ): (Tree, List[Tree]) = {
  ...
  def enumName(ownerName: String, fieldName: String, caseName: String, splitOnUnderscore: Boolean): Select = {
    Select(Select(Ident(TermName(ownerName)), TermName(fieldName.capitalize + "Enums")),
           TermName(nameFromJson(caseName, splitOnUnderscore))
    )
  }
   ...
   */
  def mkUnionTypeDef(path: List[String],
                     baseName: String,
                     schemas: List[Root],
                     unionSuffix: Boolean = true,
                     splitOnUnderscore: Boolean = false
  ...
  def mkDef(path: List[String],
    ...
    (schema.$ref, schema.enum, schema.typ, schema.oneOf, schema.multiOf) match {
      ...
      case (_, _, Some(ListSimpleTypeTyp(list)), _, _) =>
  ...
  def mkType(path: List[String],
    ...
    (schema.typ, schema.$ref, schema.enum, schema.oneOf, schema.multiOf) match {
      ...
      case (Some(SimpleTypeTyp(SimpleTypes.Object)), _, _, _, _) | (_, _, Some(_), _, _) | (_, _, _, Some(_), _) |
          (_, _, _, _, Some(_)) => {
   ...
   */
  def mkValDef(path: List[String],
               field: Field,
               optional: Boolean,
               defval: Option[Select] = None,
               unionSuffix: Boolean = true,
               splitOnUnderscore: Boolean = false
  ): (ValDef, List[Tree]) = {
